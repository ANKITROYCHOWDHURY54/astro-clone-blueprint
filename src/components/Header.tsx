...
const Header = () => {
  ...
  const menuItems = [
    "Home", "About", "Services", "Courses", "Podcast", "Blog", "FAQs", "Contact"
  ];

  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? "bg-primary/95 backdrop-blur-md shadow-mystical" : "bg-primary"
      }`}
    >
            ...
            <nav className="py-4">
              {menuItems.map((item) => (
                <a
                  key={item}
                  href={`#${item.toLowerCase()}`}
                  className="block py-2 px-4 text-primary-foreground hover:text-accent transition-colors"
